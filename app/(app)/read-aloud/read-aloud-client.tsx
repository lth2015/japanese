    ...
    <div className="px-6 lg:px-12 py-8 lg:py-12 max-w-3xl mx-auto">
      ...
      <header className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-2 text-sm text-text-secondary">
          ...
          {!current.isUnlocked && (
            <Badge variant="outline">
              提示：先在 Drill 解锁这句到 Stage 2，效果更好
            </Badge>
          )}
        </div>
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-text-muted tabular">本轮已完成 {completedIds.length}</span>
        </div>
      </header>

      {/* Sentence */}
      <section className="my-8 py-10 border-y border-border text-center">
        <p className="font-jp-serif text-text-primary leading-tight text-display">
          <FuriganaText text={s.japanese} tokens={s.tokens} showRuby={!hideFurigana} />
        </p>
        <p className="text-text-secondary text-base mt-6" lang="zh-CN">
          {s.chinese}
        </p>
      </section>
